...
window.YSApi = (() => {
  ...
  (() => {
    try {
      if (location.hash.startsWith('#session=')) {
        ...
        history.replaceState(null, '', location.pathname + location.search);
      } else if (location.hash.startsWith('#checkout=')) {
        loginNotice = 'checkout:' + location.hash.slice(10);
        history.replaceState(null, '', location.pathname + location.search);
      }
    } catch (e) { /* ignore */ }
  })();
  ...
  /* ===== billing (Stripe via the Worker) ===== */

  /** Start Stripe Checkout for 'pro' or 'pass'. Throws with a message when the
   * user isn't signed in (the sheet shows the sign-in form for that). */
  async function checkout(plan) {
    ...
    const data = await r.json().catch(() => ({}));
    if (r.status === 401) { const e = new Error('sign_in_required'); e.code = 401; throw e; }
    if (!r.ok || !data.url) throw new Error(data.message || 'Could not start checkout.');
    location.href = data.url; // off to Stripe
  }

  /** Open the Stripe customer portal (cancel / update card / invoices). */
  async function portal() {
    const r = await api('/v1/billing/portal', { method: 'POST' });
    const data = await r.json().catch(() => ({}));
    if (!r.ok || !data.url) throw new Error(data.message || 'Could not open billing.');
  ...
  function renderAccountUi() {
    ...
    if (currentMe) {
      ...
      $('account-tier').textContent = currentMe.tier === 'pro' ? 'Pro' : 'Free plan';
      const billing = $('account-billing');
      if (billing) billing.style.display = currentMe.hasBilling ? '' : 'none';
      // Signed in but not Pro: surface the checkout button for the selected plan.
      const co = $('account-checkout');
      if (co) co.style.display = currentMe.tier === 'pro' ? 'none' : '';
    } else {
      const billing = $('account-billing');
   ...
   * poll /v1/me until the tier flips (or give a patient message). */
  async function pollForUpgrade() {
    setStatus('Payment received — activating Pro\u2026');
    for (let i = 0; i < 10; i++) {
      await new Promise((res) => setTimeout(res, 3000));
      await refreshAuth();
      if (currentMe && currentMe.tier === 'pro') { setStatus('Pro is active. Enjoy the yard.'); return; }
    }
    setStatus('Payment received. Pro can take a minute to activate — pull to refresh shortly.');
  }
  ...
  function init(hooks) {
    ...
    const billing = $('account-billing');
    if (billing) billing.addEventListener('click', () => portal().catch((e) => setStatus(e.message, true)));
    renderAccountUi();
    if (enabled()) {
      ...
      if (loginNotice === 'checkout:success') refreshAuth().then(pollForUpgrade);
      else if (loginNotice === 'checkout:cancel') setStatus('Checkout canceled — no charge.');
      else refreshAuth();
    }
  }
