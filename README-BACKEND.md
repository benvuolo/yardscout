...
### 5. Stripe (real payments — Pro subscription + Weekend Pass)

1. Create an account at [stripe.com](https://stripe.com). You can build and
   test everything in **test mode** first (toggle in the dashboard) — no
   business verification needed until you flip live.
2. Dashboard → Product catalog → create two products:
   - **YardScout Pro** — recurring price, $12.99/month. Copy the price id
     (`price_...`).
   - **YardScout Weekend Pass** — one-time price, $6.99. Copy the price id.
3. Paste both ids into `backend/wrangler.toml` (`STRIPE_PRICE_PRO`,
   `STRIPE_PRICE_PASS`). Price ids aren't secrets — commit them.
4. Dashboard → Developers → API keys → copy the **secret key** (`sk_test_...`
   in test mode), then:
...
5. Dashboard → Developers → Webhooks → Add endpoint:
   - URL: `https://yardscout-api.<your-subdomain>.workers.dev/v1/stripe/webhook`
...
6. Dashboard → Settings → Billing → Customer portal → activate it (this powers
   the "Manage billing" button: cancel, change card, view invoices — zero
   support burden on you).
7. Test the full loop in test mode: open the app with `?api=1&apibase=...`,
   sign in, pick a plan, "Continue to payment", pay with Stripe's test card
   `4242 4242 4242 4242` (any future expiry/CVC). Within a few seconds the
   webhook flips the account to Pro and the app unlocks. Cancel via "Manage
   billing" and confirm Pro lapses.
8. Going live: flip the dashboard to live mode, repeat steps 2–6 with the
   live key/price ids/webhook secret. (Live mode requires your ID + a bank
   account for payouts — Stripe walks you through it.)

How entitlements work: the **webhook is the source of truth**. Checkout
redirects (`#checkout=success`) are only UI feedback — nobody can self-grant
Pro by visiting a URL. Every change is audited in `entitlement_events`.
Weekend Pass = the `pro` tier with a 72-hour expiry; subscriptions get a
